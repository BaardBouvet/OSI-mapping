...
| [SPARQL-IMPLEMENTATION-PLAN.md](SPARQL-IMPLEMENTATION-PLAN.md) | Superseded | Original UPDATE-based SPARQL slicing. Superseded by [SPARQL-CONSTRUCT-ARTIFACTS-PLAN.md](SPARQL-CONSTRUCT-ARTIFACTS-PLAN.md) which switched to a CONSTRUCT-only artifact pipeline shared by tests and deployment. |
| [SPARQL-CONSTRUCT-ARTIFACTS-PLAN.md](SPARQL-CONSTRUCT-ARTIFACTS-PLAN.md) | Done | CONSTRUCT-only SPARQL backend: one CONSTRUCT per derived named graph, `# Maintains: GRAPH <IRI>` annotation per query, in-process Oxigraph executor runs the same artifacts the deployer registers. Covers IDENTITY / FORWARD (coalesce + last_modified) / LIST (cells + heads) / REVERSE (existing + inserts) / FRAMING. |
| [TRIPLESTORE-BACKEND-DESIGN.md](TRIPLESTORE-BACKEND-DESIGN.md) | Draft | RDF/SPARQL renderer design — IRI scheme, named graphs, JSON-LD lift, identity / canonical / reverse stages. Companion to [SPARQL-CONSTRUCT-ARTIFACTS-PLAN.md](SPARQL-CONSTRUCT-ARTIFACTS-PLAN.md). |
| [V2-SPEC-DRAFT.md](V2-SPEC-DRAFT.md) | Draft | Backend-neutral simplification of the OSI mapping schema — informs both the PG and SPARQL renderers. |
| [V2-MIGRATION-RFC.md](V2-MIGRATION-RFC.md) | Draft | Clause-by-clause migration mapping from v1 to [V2-SPEC-DRAFT.md](V2-SPEC-DRAFT.md). |
| [V2-PROTOTYPE-EXAMPLES.md](V2-PROTOTYPE-EXAMPLES.md) | Draft | Three existing examples translated to the proposed v2 schema. |
| [VALUE-MAP-RFC.md](VALUE-MAP-RFC.md) | Committed to v2 | Field-level `value_map:` specification — mutually exclusive with `transform:`, with `value_map_fallback`. |
| [ROADMAP-RESTRUCTURE-PLAN.md](ROADMAP-RESTRUCTURE-PLAN.md) | Done | Reshape `ROADMAP.md` around numbered releases (0.1 current, 0.2 planned, 0.3 considered) and move engine-only design files out of `docs/design/` into `engine-rs/plans/`. |
| [DELTA-CHANGES-VIEW-PLAN.md](DELTA-CHANGES-VIEW-PLAN.md) | Proposed | Add unified `<M>_changes` view (`UNION ALL` over the three delta primitives with `_action` column) for stream-style consumers, recovering the v1 single-view ergonomics without conflating per-kind shapes. |
| [IVM-CONSISTENCY-PLAN.md](IVM-CONSISTENCY-PLAN.md) | Design | Diamond consistency under eventual-consistent IVM (PG mat-views, pg_ivm, Materialize, RisingWave): atomic snapshots, topological refresh, generation tokens, versioned snapshots. |
